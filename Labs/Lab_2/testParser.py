...
class testParser ( Parser ):
    ...
    class ExprContext(ParserRuleContext):
        ...
        def COMMAND(self):
            return self.getToken(testParser.COMMAND, 0)

        def SPACE(self, i:int=None):
            if i is None:
                return self.getTokens(testParser.SPACE)
            else:
                return self.getToken(testParser.SPACE, i)

        def REG(self, i:int=None):
            if i is None:
                return self.getTokens(testParser.REG)
            else:
                return self.getToken(testParser.REG, i)

        def COMMA(self):
            return self.getToken(testParser.COMMA, 0)

        def MEMORY(self):
            return self.getToken(testParser.MEMORY, 0)

        def IMMEDIATE(self):
            return self.getToken(testParser.IMMEDIATE, 0)
    ...
    def expr(self):
        ...
        try:
            ...
            if la_ == 1:
                ...
                self.state = 4
                self.match(testParser.COMMAND)
                self.state = 5
                self.match(testParser.SPACE)
                self.state = 6
                self.match(testParser.REG)
                self.state = 7
                self.match(testParser.COMMA)
                self.state = 8
                self.match(testParser.SPACE)
                self.state = 9
                self.match(testParser.MEMORY)
                pass
                ...
                self.match(testParser.SPACE)
                self.state = 27
                self.match(testParser.IMMEDIATE)
                pass
