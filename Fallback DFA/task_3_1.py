...
if __name__ == '__main__':
    ...
    state_expression_dict[((state_expression.split('),')[-1]).split(","))[0].strip()[1:]] = ((state_expression.split('),')[-1]).split(","))[1].strip()[1:-2]
    ...
    expressions_dict[((expressions.split('),')[-1]).split(","))[0].strip()[2:-1]] = ((expressions.split('),')[-1]).split(","))[1].strip()[1:-2]
    ...
    if(states_stack.peek() in end_states):
        ...
        while states_stack.size() > 0:
            ...
            if(curr in end_states):
                ...
                last_line = lines[R:L + 1]

        if(last != ""):
           
            result += last_line + ", " + expressions_dict[state_expression_dict[last]] + "\n"
