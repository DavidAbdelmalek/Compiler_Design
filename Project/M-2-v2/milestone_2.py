...
def get_token_type(token):
    if token.type == milestone_2Lexer.AND:
        return "AND"
        ...
        return "IN"
    elif token.type == milestone_2Lexer.INCLUDE:
        return "INCLUDE"
        ...
        return "BIN_LIT"
    elif token.type == milestone_2Lexer.HEXDIGIT:
        return "HEXDIGIT"
    elif token.type == milestone_2Lexer.OCTDIGIT:
        return "OCTDIGIT"
    elif token.type == milestone_2Lexer.BINDIGIT:
        return "BINDIGIT"
...
def main():
    ...
    cat_err = ChatErrorListener('')
    parser._listeners = [cat_err]
    tree = parser.start()
   # print(Trees.toStringTree(tree, None, parser))

    # print(cat_err.output)
    out_file = open("milestone_2_result.txt", "w")
